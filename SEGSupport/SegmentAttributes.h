...
unsigned* getRGBFromCIEXYZ(float* cieXYZ, unsigned *rgb) {
   ...
   rgb[0] = (int)round(var_R * 255);
   rgb[1] = (int)round(var_G * 255);
   rgb[2] = (int)round(var_B * 255);
...
unsigned* getIntegerScaledCIELabFromCIELab(float *cieLab, unsigned *cieLabScaled){
    ...
    cieLabScaled[0] = (int)round (cieLab[0] * 65535 / 100);
    cieLabScaled[1] = (int)round((cieLab[1] + 128) * 65535 / 255);
    cieLabScaled[2] = (int)round((cieLab[2] + 128) * 65535 / 255);
    return cieLabScaled;
}
